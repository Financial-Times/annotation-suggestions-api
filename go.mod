module github.com/Financial-Times/draft-content-suggestions

go 1.14

require (
	github.com/Financial-Times/api-endpoint v1.0.0-rc1
	github.com/Financial-Times/go-ft-http v1.0.0-rc1
	github.com/Financial-Times/go-fthealth v0.0.0-20181009114238-ca83ad65381f
	github.com/Financial-Times/go-logger/v2 v2.0.1
	github.com/Financial-Times/http-handlers-go/v2 v2.2.0
	github.com/Financial-Times/service-status-go v0.0.0-20160323111542-3f5199736a3d
	github.com/Financial-Times/transactionid-utils-go v0.2.0
	github.com/gorilla/mux v1.7.4
	github.com/jawher/mow.cli v1.1.0
	github.com/niemeyer/pretty v0.0.0-20200227124842-a10e7caefd8e // indirect
	github.com/onsi/ginkgo v1.12.0 // indirect
	github.com/onsi/gomega v1.9.0 // indirect
	github.com/rcrowley/go-metrics v0.0.0-20180125231941-8732c616f529
	github.com/satori/go.uuid v1.2.1-0.20181016170032-d91630c85102
	github.com/sirupsen/logrus v1.0.5
	github.com/stretchr/testify v1.5.1
	golang.org/x/crypto v0.0.0-20180127211104-1875d0a70c90 // indirect
	gopkg.in/check.v1 v1.0.0-20200227125254-8fa46927fb4f // indirect
)
